...
class hexprintrasterizer{
	...
	double linedist(vec2 p, vec2 p0, vec2 v, float len){
		...
		vec2 c = p0+v*t1; // closest point
		vec2 d = c-p;
		return d.x*d.x+d.y*d.y;
		
	}
	double thex(int x,int y, vec2 & p)
	{
		x=(x+GWIDTH)%GWIDTH;
		y=(y+GHEIGHT)%GHEIGHT;
		int k= grid[y][x];
		linedist(p,indexToPoint(x,y)+tp[k]*(float)r,(tp[k+3]-tp[k])*.5f,2.0*r);
	}
public:
	hexprintrasterizer(){};
	void rasterize()
		...
		for(int j = 0; j<HEIGHT; j++){
			for(int i = 0; i<WIDTH; i++){
				...
				if(k<=2){
					...
					//double t = linedist(vec2(i,j),vec2(hi,hj),(tp[k+3]-tp[k])*(float)r);
					//double t = linedist(vec2(i,j),vec2(WIDTH/2,HEIGHT/2),(tp[k+3]-tp[k])*(float)r);
					//double t = linedist(vec2(i,j),vec2(WIDTH/2,HEIGHT/2),vec2(100,0));

					if((sqrt(t1)+sqrt(t2))<r+2)
						g=1.0;

					imageFrame->set( i, j,cos(r/(sqrt(t1)+1.0))*-.5+.5,0,0 );
				}
				else
